...
**Secure Practices and Considerations**

1) API Rate Limiting:
- Implemented using slowapi to prevent abuse or overloading of the service.
    - Root Endpoint (/): Limited to ROOT_RATE_LIMIT requests per minute as configured in config.yaml.
    - Prediction Endpoint (/predict/): Limited to PREDICT_RATE_LIMIT requests per minute to safeguard against resource exhaustion.

2) License Plate Detection and Blurring:
    - Integrated OpenCV-based detection and blurring of license plates through the detect_and_blur_license_plate function.
    - **Purpose**: To anonymize sensitive information and comply with data privacy standards if required. Can be configured in the config.yaml as **enable_plate_detection**

3) Input Image Validation:
    - Important for protection against malicious files and balance resource usage.
    - Ensured that the file uploaded is of **.jpg** or **.png** formats and that the file size is capped at 5MB.
...
# Key Insights and Takeaways (See model_training_pipeline.ipynb for more details)
**Starting Point**: 
- Freezing all layers in ResNet-18 except the classification head (FC Layer)
- Learning Rate of 0.001
- Adam optimizer and no dropout. 

**Result**: High Training Accuracy (Low Training Loss), but Training accuracy plateaued early (High Training Loss).

Subsequent Experiments:
- List of changes were made below. For each hyperparameter settings, the train and validation loss and accuracy were used to assess the model's performance.

### Experiment Summary and Insights

| **Experiment**                                      | **Change Made**                          | **Reason**                                                | **Insight**                                                                 |
|-----------------------------------------------------|-----------------------------------------|----------------------------------------------------------|-----------------------------------------------------------------------------|
| **Baseline (Freeze Inner Layer, Train FC layer)**   | All layers frozen, no dropout           | Set Baseline                                             | Overfitting observed with high training accuracy and low generalization. Model Performance was poor.    |
...
There were cars that have a high misclassification rate (See notebook for more analysis)
An example is the misclassification amongst Audi Cars. Table below shows some samples:

| True Label         | Predicted Label      | Misclassification Count |
|---------------------|----------------------|-------|
